ALTER TABLE public.calendar_events ADD COLUMN IF NOT EXISTS notify_slack boolean NOT NULL DEFAULT false;
...
AS $function$
begin
  if coalesce(new.notify_slack, false) is not true then
    return new;
  end if;
