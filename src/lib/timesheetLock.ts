 ...
 *
 * A submitted timesheet stays editable by the contractor until 6:00 AM Eastern
...
/** Instant for a given ET calendar date at 06:00 ET. */
const etSixAm = (year: number, month: number, day: number): Date => {
  const guess = new Date(Date.UTC(year, month - 1, day, 6, 0, 0));
...
/**
 * First Sunday 6:00 AM ET strictly after the given instant.
 */
export const nextSundaySixAmEt = (after: Date): Date => {
  for (let i = 0; i <= 8; i++) {
    const candidateDay = new Date(after.getTime() + i * 86400000);
    const p = etParts(candidateDay);
    if (p.weekday !== 0) continue;
    const lock = etSixAm(p.year, p.month, p.day);
...
/**
 * When a timesheet for the given week-ending date locks: 6:00 AM ET on the
...
export const timesheetLockAt = (weekEndingDate: string, submittedAt?: string | null): Date => {
  ...
  const endOfWeekDay = new Date(etSixAm(p.year, p.month, p.day).getTime() + 18 * 3600000); // ~midnight ET
