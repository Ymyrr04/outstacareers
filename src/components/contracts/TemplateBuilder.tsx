...
export const TemplateBuilder = ({ templateId, onBack }: { templateId: string; onBack: () => void }) => {
  ...
  const saveAll = async () => {
    ...
    try {
      ...
      for (const f of dirty) {
        ...
        const { error } = await supabase.from("contract_template_fields").upsert(payload);
      ...
      <div className="flex items-center justify-between gap-3 sticky top-0 z-20 bg-background py-2 border-b">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Click a field type below, then click on the PDF to place it. Drag to move.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-[1fr_280px] gap-4">
        ...
        <div className="space-y-4">
          <Card className="p-3">
            ...
            <p className="text-xs text-muted-foreground mb-3">Click a button, then click on the PDF.</p>
            <div className="grid grid-cols-2 gap-2">
              {(Object.keys(FIELD_DEFAULTS) as FieldType[]).map(t => {
                const Icon = FIELD_DEFAULTS[t].icon;
                return (
                  <Button key={t} variant={pendingType === t ? "default" : "outline"} size="sm" onClick={() => setPendingType(pendingType === t ? null : t)} className="gap-1 justify-start">
          ...
          {selected && (
            <Card className="p-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div>W: <Input type="number" step="0.01" value={selected.width_pct.toFixed(3)} onChange={(e) => updateField(selected.id, { width_pct: +e.target.value })} className="h-7" /></div>
                <div>H: <Input type="number" step="0.01" value={selected.height_pct.toFixed(3)} onChange={(e) => updateField(selected.id, { height_pct: +e.target.value })} className="h-7" /></div>
              </div>
            </Card>
          )}
        </div>
  ...
  // helper - pending type state (declared via inline scope hack: convert to top-level)
};

// Lift state out of body
let _pendingTypeShim = null as FieldType | null;
let pendingType: FieldType | null = null;
let _setPendingType: any = () => {};
function setPendingType(v: FieldType | null) { _setPendingType(v); }

// Replace the implementation with proper React hook usage
function PageCanvas({
  ...
  return (
    <div
      ref={registerRef}
      onClick={handleClick}
      className={`relative bg-background border rounded shadow-sm mx-auto ${pendingType ? "cursor-crosshair" : ""}`}
